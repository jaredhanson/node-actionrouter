...
Router.prototype.resources = function(name, options, fn) {
  ...
  actions.forEach(function(action) {
    switch (action) {
      ...
      case 'create':  self._route('post', path                       , controller, 'create' );
        break;
      case 'show':    self._route('get' , path + '/:id.:format?'     , controller, 'show'    , helper);
        break;
      case 'edit':    self._route('get' , path + '/:id/edit.:format?', controller, 'edit'    , utils.functionize('edit', helper));
        break;
