...
Router.prototype.match = function(pattern, shorthand, opts) {
  ...
  for (var i = 0, len = methods.length; i < len; i++) {
    ...
    if (typeof shorthand === 'function' || Array.isArray(shorthand)) {
      // TODO: allow `as` option for middleware routes
...
Router.prototype.resource = function(name, options, fn) {
  ...
  actions.forEach(function(action) {
    switch (action) {
      case 'new':     self._route('get' , path + '/new.:format?' , controller, 'new'     , { helper: utils.functionize('new', helper) });
        break;
      case 'create':  self._route('post', path                   , controller, 'create' );
        break;
      case 'show':    self._route('get' , path + '.:format?'     , controller, 'show'    , { helper: helper });
        break;
      case 'edit':    self._route('get' , path + '/edit.:format?', controller, 'edit'    , { helper: utils.functionize('edit', helper) });
        break;
...
Router.prototype.resources = function(name, options, fn) {
  ...
  actions.forEach(function(action) {
    switch (action) {
      ...
      case 'show':    self._route('get' , path + '/:id.:format?'     , controller, 'show'    , { helper: helper });
        break;
      case 'edit':    self._route('get' , path + '/:id/edit.:format?', controller, 'edit'    , { helper: utils.functionize('edit', helper) });
        break;
  ...
  // TODO: This should be underscore?
  placeholder = ':' + utils.functionize(singular) + '_id';
...
Router.prototype.find = function(controller, action) {
  console.log('FINDING ROUTE: ' + controller + ' ' + action);
  console.log(this._entries)
  
  // FIXME: Locomotive is still passing in 'FooController' style strings
  var key = utils.controllerize(controller) + '#' + action;
